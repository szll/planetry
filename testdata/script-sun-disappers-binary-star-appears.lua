...
function letSunDisappear()
  ...
  if steps >= 365 and not swapDone then
    ...
    local halfDistance = 0.1 * AU -- astronomical unit

    -- Add a binary star where the sun was, both stars should be stable ...
    local alpha = createBody("alpha", "Alpha", mass, 1, createPoint3D(0, halfDistance, 0), createVector3D(velocity, 0, 0))
    addBodyToScene(alpha, 255, 0, 255, 255)
    print("created alpha", steps)
    
    local beta = createBody("beta", "Beta", mass, 1, createPoint3D(0, -halfDistance, 0), createVector3D(-velocity, 0, 0))
